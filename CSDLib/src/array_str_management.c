...
char **create_str_array_from_file(char *file_name)
{
    ...
    char **str_array = my_malloc(sizeof(char *) * (get_nb_lines(file_name) + 1));
    char *str = NULL;
    size_t s = 0;
...
{
    char **new_arr = my_malloc(sizeof(char *) * (arr_size + 1));

    for (int i = 0; i < arr_size; i++) {
        new_arr[i] = my_malloc(sizeof(char));
        new_arr[i] = arr[i];
    }
...
char **push_front_str(char **arr, char *str, int arr_size)
{
    ...
    for (int i = 0; i < arr_size; i++) {
        new_arr[i+1] = my_malloc(sizeof(char));
        new_arr[i+1] = arr[i];
    }
    new_arr[0] = my_malloc(sizeof(char));
    new_arr[0]= str;
    return (new_arr);
}
...
char **insert_str(char **arr, char *str, int arr_size, int val_pos)
{
    ...
    for (int i = 0; i < arr_size; i++) {
        if (i<val_pos) {
            ...
            new_arr[val_pos] = my_malloc(sizeof(char *));
            new_arr[val_pos] = str;
        }
        else {
            new_arr[i] = my_malloc(sizeof(char *));
...
{
    char **array = my_malloc(sizeof(char *) * (get_nb_words(str, separator) + 1));
    int is_sep = 0;
    int row = 0;
    int nb_chars = 0;
    int to = 0;
    ...
    for (int i = 0; str[i] != '\0'; i++) {
        ...
        if ((is_sep == 1 && nb_chars > 0) || (is_sep == 0 && str[i+1] == 0)) {
            array[row] = my_malloc(sizeof(char) * nb_chars + 1);
            if (is_sep == 0)
                to = 1;
